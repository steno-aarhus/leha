...
oral_defence <- sorted_data %>% mutate(
  pdff9_2014 = ifelse (p40061_i2 >= 9, 1, 0),
  pdff9_2014 = as.numeric(pdff9_2014),
  pdff9_2019 = ifelse (p40061_i3 >= 9, 1, 0),
  pdff9_2019 = as.numeric(pdff9_2019)
)


# Analyses
create_formula <- function(xvars, covars) {
  outcome <- "Surv(survival_time, pdff9_2014 == 1)"
  reformulate(c(xvars, covars), response = outcome)
}
...
main_model2<- function(data) {
  ...
  model2_results <- model2_formulas |>
    map(~ coxph(.x, data = known_history, ties = "breslow")) |>
